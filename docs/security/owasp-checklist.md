...
| 6   | Vulnerable & Outdated Components         | Applicable — the real ongoing risk for this project.                                                       | ✅ `npm audit --audit-level=high` in CI · ✅ Automated dependency-update PRs via `.github/dependabot.yml` (npm + github-actions, weekly)                                                                                                 |
| 7   | Identification & Authentication Failures | N/A — no accounts or sessions exist.                                                                       | —                                                                                                                                                                                                                                        |
| 8   | Software & Data Integrity Failures       | Applicable to the CI/CD supply chain.                                                                      | ✅ `package-lock.json` committed, `npm ci` used for reproducible installs · ✅ CI secret-scanning (`gitleaks`) in both `ci.yml` and `.circleci/config.yml`                                                                               |
...
| `security.txt` for responsible disclosure                   | ✅ `public/.well-known/security.txt`, RFC 9116                                                  |
